...
def main():
    ...
    # Use a smaller directory for faster search
    benchmarks = [
        ("1. read this project", lambda: summarize_project("kryth/src")),
        ("2. explain app.py", lambda: "READ: explain app.py"),
        ("3. find routes", lambda: run_search("find routes", "kryth/src")),
        ("4. create hello.py", lambda: "MODIFY: create hello.py"),
        ("5. run this project", lambda: detect_stack(".")),
        ("6. trace auth flow", lambda: run_search("trace auth flow", "kryth/src")),
    ]
