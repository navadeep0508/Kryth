...
if 'init' in sys.argv:
	from browser_agent.init_cmd import INIT_TEMPLATES
	from browser_agent.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'browser_agent_{template}.py'
...
from browser_agent import Agent, Controller
from browser_agent.agent.views import AgentSettings
from browser_agent.browser import BrowserProfile, BrowserSession
from browser_agent.logging_config import addLoggingLevel
from browser_agent.telemetry import CLITelemetryEvent, ProductTelemetry
from browser_agent.utils import get_browser_agent_version
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('browser_agent.'):
		...
		# Configure browser_agent logger and all its sub-loggers
		browser_agent_logger = logging.getLogger('browser_agent')
		browser_agent_logger.propagate = False  # Don't propagate to root logger
		browser_agent_logger.handlers = [log_handler]  # Replace any existing handlers
		browser_agent_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['browser_agent.Agent', 'browser_agent.controller', 'browser_agent.agent', 'browser_agent.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'browser_agent.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('browser_agent.on_mount')
		...
		self._telemetry.capture(
			CLITelemetryEvent(
				version=get_browser_agent_version(),
				action='start',
				mode='interactive',
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent browser_agent from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('browser_agent.startup')
	...
	try:
		...
		try:
			from browser_agent.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from browser_agent.sync.auth import DeviceAuthClient
	...
	try:
		...
		from browser_agent.sync.service import CloudSync
		...
		if task_id and sync_service:
			try:
				from browser_agent.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: browser_agent_<template>.py)')
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'browser_agent_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: browser_agent_<template>.py)',
)
@click.option(
	'--force',
	'-f',
