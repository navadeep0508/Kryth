...
from browser_agent.observability import observe
...
class ChatBrowserUse(BaseChatModel):
		...
		**kwargs,
		...
		if not self.api_key:
			raise ValueError(
				...
				'https://cloud.browser-use.com/new-api-key?utm_source=oss&utm_medium=chat_browser_agent'
			)
	...
	@observe(name='chat_browser_agent_ainvoke')
	async def ainvoke(
		self,
	...
	def _raise_http_error(self, e: httpx.HTTPStatusError) -> None:
		...
		if status_code == 401:
			...
			raise ModelProviderError(
				message=f'Browser Use credits exhausted. Add more at:\nhttps://cloud.browser-use.com/billing?utm_source=oss&utm_medium=chat_browser_agent\n{error_detail}',
				status_code=402,
				model=self.name,
			)
