...
def _wrap(title: Text, body: RenderableType, *, border: str = "divider") -> Panel:
    return Panel(body, title=title, title_align="left", border_style=border, padding=(1, 2), expand=False)
...
def create_panel(path: str, content: str) -> Panel:
    ...
    return _wrap(_file_title("write", path, kind_style="log.success"), Group(*parts))


def delete_panel(path: str) -> Panel:
    return _wrap(_file_title("delete", path, kind_style="log.error"), Text.assemble((ERROR, "log.error"), (" file removed", "log.error")), border="log.error")
...
def grouped_updates_panel(title_text: str, children: Sequence[Panel]) -> Panel:
    ...
    for i, child in enumerate(children):
        ...
        parts.append(child)
    return Panel(Group(*parts), title=Text.assemble((CORE, "kryth.core"), (" " + title_text, "section.exec")), title_align="left", border_style="divider", padding=(1, 1), expand=False)
